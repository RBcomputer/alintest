...
const firebaseConfig = {
  apiKey: "AIzaSyD9U3FXNyb27oIAYoh1wnLfiVvMS_XZvt0",
  authDomain: "alin-44851.firebaseapp.com",
  databaseURL: "https://alin-44851-default-rtdb.firebaseio.com",
  projectId: "alin-44851",
  storageBucket: "alin-44851.appspot.com",
  messagingSenderId: "228406714979",
  appId: "1:228406714979:web:920ac170cc508133a0ce56",
  measurementId: "G-JHDQCENQGB",
};
...
function inizializzaSincronizzazioneRealTime() {
  db.ref(`${APP_ID}/interventi`).on("value", (snapshot) => {
    console.log("Dati interventi aggiornati:", snapshot.val());
    caricaDati().then(() => {
      aggiornaListaClientiDaPianificare();
      aggiornaInterventiInCorso();
      aggiornaInterventiCompletati();
  ...
  db.ref(`${APP_ID}/rubrica`).on("value", (snapshot) => {
    console.log("Dati rubrica aggiornati:", snapshot.val());
    caricaDati().then(() => {
      aggiornaRubrica();
    });
  });
...
interventoForm.addEventListener("submit", (e) => {
  e.preventDefault();
  const nuovoCliente = {
    cliente: document.getElementById("cliente").value || "N/D",
    indirizzo: document.getElementById("indirizzo").value || "N/D",
    titolo: document.getElementById("titolo").value || "N/D",
    tipoIntervento: document.getElementById("tipoIntervento").value || "N/D",
  ...
  salvaDati()
    .then(() =>
      showSuccessToast("Nuovo cliente aggiunto e salvato con successo!")
    )
    .catch((error) =>
      showErrorToast("Errore nel salvataggio del nuovo cliente: " + error)
    );
});
...
function pianificaAppuntamento(button) {
  ...
  document.getElementById("modalAppuntamento").style.display = "block";
}
...
formAppuntamento.addEventListener("submit", (e) => {
  ...
  const interventoInCorso = {
    ...
    priorita:
      clienteSelezionato.cells[0].querySelector("div")?.style.backgroundColor ||
      "",
  };
